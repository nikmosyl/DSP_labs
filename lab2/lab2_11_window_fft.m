% Пример частотно-временного преобразования Фурье
clear;
...
X0 = fft(x0);
subplot(2,2,1); 
plot(x0); grid on; title('С тремя частотами в сигнале');
subplot(2,2,2);
stem(abs(X0)); grid on; title('ДПФ сигнала с тремя частотами в сигнале');
...
subplot(2,2,3);
plot(x); grid on; title('Три подряд идущие частоты');
X = fft(x);
subplot(2,2,4);
stem(abs(X)); grid on; title('ДПФ сигнала в котором присутствуют три подряд идущие частоты');
