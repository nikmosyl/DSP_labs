...
N1 = length(ts1);
N2 = length(ts2);
F1 = 21; % Частоты основных гармоник в сигнале
F2 = 42;
...
X = abs(fft(x1))*2/N1;
figure;
subplot(2,2,1); plot(x1); grid on; title('Исходный сигнал');
subplot(2,2,2); plot(F,X); grid on; title('ДПФ исходного сигнала');

xw = x1.*blackman(N1)';
subplot(2,2,3); plot(xw); grid on; title('Взвешенный окном Блэкмана');

Xw = abs(fft(xw))*2/N1;
subplot(2,2,4); plot(F,Xw); grid on; title('ДПФ сигнала, взвешенного окном Блэкмана');
